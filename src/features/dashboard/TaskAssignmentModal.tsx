...
export function DashboardTaskAssignmentModal({
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const ta = String(formData.get("assistant") ?? "담당 조교").trim();
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative flex w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-2xl dark:border-slate-800 dark:bg-[#1c2936] dark:text-white">
            ...
            <form
              className="custom-scrollbar max-h-[70vh] space-y-6 overflow-y-auto px-6 py-6"
            ...
            >
              <div className="rounded-lg border border-blue-100 bg-blue-50/80 p-3 text-xs text-blue-700 dark:border-blue-900/30 dark:bg-blue-900/10 dark:text-blue-300">
                업무 지시는 강사/조교 대시보드 상세 모달 기준으로 저장됩니다.
              </div>
              ...
              <footer className="flex items-center justify-end gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-[#151f28]">
                ...
                <button
                  type="submit"
                  className="flex items-center gap-2 rounded-lg bg-primary px-5 py-2 text-sm font-bold text-white shadow-sm transition hover:bg-primary/90"
                >
